...
async def help(ctx,page = 0):
    if ctx.message.author.bot:
        return
    if page == 0 or page > 4:
        default = '`1.` Модерация\n`2.` Музыка\n`3.` Утилиты\n`4.` Прочее\n\nИспользуйте `!!help [page]` для вывода списка команд из этой категории.'
...
async def status(ctx):
    ...
    e.add_field(name = 'Версия', value = 'b_9.9.6', inline = True)
...
async def say(ctx, *, text):
    if ctx.message.author.bot:
        return
    if not ctx.author.has_permission(mention_everyone = True):
        if re.search("@", str(text)):
            emb = discord.Embed(description = ':x: У вас нет прав для отправки сообщений с упомянаниями.', color = 0xdd2e44)
            await ctx.send(embed = emb, delete_after = 2)
            return
    await ctx.send(text)
...
async def timer(ctx, time = ''):
    ...
    try:
        ...
        emb = discord.Embed(description = ':x: Некорректный формат времени.\nПример: `!!timer 5m`', color = 0xdd2e44)
        await ctx.send(embed = emb, delete_after = 2)
        return
    if time <= 0.005:
        emb = discord.Embed(description = ':x: Некорректный формат времени.', color = 0xdd2e44)
