...
async def status(ctx):
    ...
    e.add_field(name = 'Версия', value = 'b10.2.1', inline = True)
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def about(ctx):
    emb = discord.Embed(title = "Бета 10.2.1 от 02.07.2021 17:34 MSK", color = 0x000000)
...
async def on_message(message):
    if message.author.bot:
        return
    try:
        print(f"{message.guild.name} | [{message.author.id}] {message.author}: {message.content}")
    except:
        print(f"Личные сообщения | [{message.author.id}] {message.author}: {message.content}")
