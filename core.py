...
bot = commands.Bot('!!', shard_count = 4, case_insensitive = True)
...
async def status(ctx):
    ...
    e.add_field(name = 'Версия', value = 'b10.2.2', inline = True)
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def about(ctx):
    emb = discord.Embed(title = "Бета 10.2.2 от 04.07.2021 20:16 MSK", color = 0x000000)
    fixed = "ㆍПри использовании отрицательного времени `!!mute` выдает мут навсегда, а не на отрицательное количество времени."
    improved = "ㆍПереработаны команды `!!radio` и `!!now`: теперь вместо ссылки бот выводит название радиостанции.\nㆍСлегка изменен вывод сообщений об ошибках."
    added = "ㆍКоманда `!!server` для получения информации о сервере.\nㆍКоманда `!!nekogif` - случайная гифка с неко.\nㆍБот выдает сообщение об ошибке при невозможности выполнения команды в канале личных сообщений."
