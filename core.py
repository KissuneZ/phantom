...
async def help(ctx, page = 0):
    if ctx.message.author.bot:
        return
    if page == 0 or page > 4:
        default = '`1.` Модерация\n`2.` Музыка\n`3.` Утилиты\n`4.` Прочее\n\nИспользуйте `!!help [page]` для просмотра списка команд из этой категории. Что нового - `!!about`.'
...
async def status(ctx):
    ...
    e.add_field(name = 'Версия', value = 'b10.1.1', inline = True)
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def about(ctx):
    emb = discord.Embed(title = "Бета 10.1.1 от 29.06.2021 19:26 MSK", color = 0x000000)
    fixed = "ㆍНебольшие исправления в команде `!!unban`"
    improved = "ㆍВместо замены любых упоминаний при использовании `!!say`, бот запрещает отправку сообщения, содержащего упоминания, если у пользователя отсутствует право «Упоминание \@everyone, \@here и всех ролей»."
    added = "ㆍКоманда `!!skin` для просмотра скинов Minecraft."
...
async def volume(ctx, volume: int):
    ...
    if not ctx.author.voice:
        ...
        return
    if not 0 <= volume <= 100:
        emb = discord.Embed(description = ':x: Громкость звука должна быть в пределах от 0 до 100.', color = 0xdd2e44)
        await ctx.send(embed = emb, delete_after = 4)
        return
    if voice:
        key = ctx.guild.id
        if Volumes.get(key) == None:
            Volumes[key] = volume
            v = volume
        else:
            vol = Volumes[key]
        ...
        try:
            voice_client.source = discord.PCMVolumeTransformer(voice.source, volume = 1.0)
        except:
            emb = discord.Embed(description = ':x: Сейчас ничего не играет.', color = 0xdd2e44)
            await ctx.send(embed = emb, delete_after = 2)
            return
        voice_client.source.volume = volume / 100
        emb = discord.Embed(description = f'<:phantom_ok:837302406060179516> Громкость звука установлена на {v}%.', color = 0x000000)
        await ctx.send(embed = emb)
    else:
        emb = discord.Embed(description = ':x: Я не подключен к голосовому каналу.', color = 0xdd2e44)
        await ctx.send(embed = emb, delete_after = 2)
        return
...
async def unmute(ctx, member: discord.Member):
    ...
    if not muterole in member.roles:
        ...
        await ctx.send(embed = emb, delete_after = 2)
        return
    emb = discord.Embed(description = f'<:phantom_ok:837302406060179516> {member.mention} размучен.',color=0x000000)
    try:
        await member.remove_roles(muterole)
    except:
        await permerror(ctx)
...
async def kick(ctx, member: discord.Member):
    if member == ctx.author:
        ...
        await ctx.send(embed = emb, delete_after = 2)
        return
    emb = discord.Embed(description = f'<:phantom_ok:837302406060179516> {member.mention} кикнут.', color = 0x000000)
    try:
        await member.kick()
    except:
        await permerror(ctx)
...
async def ban(ctx, member: discord.Member):
    if member == ctx.author:
        ...
        await ctx.send(embed = emb, delete_after = 2)
        return
    emb = discord.Embed(description = f'<:phantom_ok:837302406060179516> {member} забанен.', color = 0x000000)
    try:
        await member.ban()
    except:
        await permerror(ctx)
...
async def unban(ctx, *, member):
    ...
    try:
        ...
        await ctx.send(embed = emb, delete_after = 2)
        return
    emb = discord.Embed(description = f'<:phantom_ok:837302406060179516> {user} разбанен.', color  = 0x000000)
    await ctx.send(embed = emb)
